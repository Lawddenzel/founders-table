  ...
  try {
    ...
    await resend.emails.send({
      ...
      html: `
        ...
        <p><strong>Seats remaining:</strong> ${SEAT_LIMIT - value - 1}</p>
      `,
    });
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p>Further details will be shared closer to the event. In the meantime, feel free to reach out to us at <a href="mailto:info@unqodigital.com">info@unqodigital.com</a>.</p>
          <p style="color: #888; font-size: 13px; margin-top: 32px;">Unqo Digital Solutions CC · Windhoek, Namibia</p>
        </div>
      `,
    });

    return NextResponse.json({ success: true, seatsRemaining: SEAT_LIMIT - value - 1 });
  } catch (error) {
    console.error(error);
